...
# Tensor Core FLOPS per SM per cycle for different architectures and data types
# Format: {(major, minor): {dtype: flops_per_sm_per_cycle}}
# Reference: NVIDIA Architecture Whitepapers
_TC_FLOPS_PER_SM_PER_CYCLE = {
    # Ampere (A100)
    (8, 0): {
        "fp64": 32,  # FP64 Tensor Core
        "fp32": 256,  # TF32 (actually uses TF32)
        "tf32": 256,
        "fp16": 512,
        "bf16": 512,
        "int8": 1024,
...
# Known GPU peak specs (for validation/fallback)
# Format: {gpu_name_pattern: {dtype: peak_tflops}}

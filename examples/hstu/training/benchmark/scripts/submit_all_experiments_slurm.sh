...
for i in "${!EXP_NAMES[@]}"; do
    ...
    EXPORT_VARS="ALL,EXP_NAME=${exp},GIN_OPTIONS=${gin_opts},EXP_OUTPUT_DIR=${EXP_OUTPUT_DIR},ENABLE_NSYS=${ENABLE_NSYS},HSTU_ROOT=${HSTU_ROOT},CONTAINER_IMAGE=${CONTAINER_IMAGE},MEM_DEBUG=${MEM_DEBUG},CUDA_MEM_WATCHDOG=${CUDA_MEM_WATCHDOG},CUDA_MEM_WATCHDOG_THRESHOLD=${CUDA_MEM_WATCHDOG_THRESHOLD:-0.5},CACHE_DEBUG=${CACHE_DEBUG},CUDA_DEVICE_MAX_CONNECTIONS=${CUDA_DEVICE_MAX_CONNECTIONS:-8},RECSYS_PIPELINE_BACKEND=${RECSYS_PIPELINE_BACKEND:-legacy},HSTU_AUTOSCHED_COST_FILE=${HSTU_AUTOSCHED_COST_FILE:-},HSTU_AUTOSCHED_MAX_IN_FLIGHT=${HSTU_AUTOSCHED_MAX_IN_FLIGHT:-5}"
    if [ -n "${HSTU_PIPELINE_CONFIG:-}" ]; then
        EXPORT_VARS="${EXPORT_VARS},HSTU_PIPELINE_CONFIG=${HSTU_PIPELINE_CONFIG}"
    fi

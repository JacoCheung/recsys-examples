...
def _compute_deltas(
    experiments_txt: Optional[str], sorted_exp_names: list
) -> list:
...
def _plot_single_metric(
    ...
    if plot_type == "bar":
        ...
        for idx, val in enumerate(values):
            ax.annotate(
                fmt_func(val), (idx, val),
                textcoords="offset points", xytext=(0, 10),
                ha="center", fontsize=9, fontweight="bold",
            )
            if delta_labels and delta_labels[idx]:
                ax.annotate(
                    delta_labels[idx], (idx, val),
...
def plot_comparison(
    ...
    bars = ax_flops.bar(
        x, flops_values, color=bar_colors, edgecolor="black", linewidth=0.5,
        zorder=2, alpha=0.85, label="TFLOPS",
    )
    ...
    ax_mfu.plot(
        x, mfu_values, "s-",
        color=mfu_color, linewidth=2.5, markersize=7,
        zorder=3, label="MFU (%)",
    )
    for idx, mv in enumerate(mfu_values):
        ax_mfu.annotate(
            ...
            ha="center", fontsize=8, color=mfu_color, fontweight="bold",
        )

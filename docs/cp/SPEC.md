...
| ~~**HSTU "heterogeneous mask"** (`num_contexts`, `num_targets`, `target_group_size > 1`)~~ | **SHIPPED** via the het-mask track (`docs/cp/het_mask_design.md`) — the FBGEMM kernel's arbitrary-mask `func` facility expresses these masks per-Q-row, and the CP wrapper builds a per-step `func` tensor that projects them onto the local DualChunkSwap layout. Forward + backward verified at cp ∈ {2, 4} against single-GPU baseline. Requires the kernel to be built with `HSTU_ARBITRARY_NFUNC ≥ 3`. |
| ~~**Sliding-causal `window_size=(w, 0)` with `w > 0`**~~ | **SHIPPED** via the same `func`-tensor path as het-mask — `_is_het_mask` returns True for any non-causal window, so the wrapper routes to `_multi_gpu_forward_arbitrary` which handles sliding the same way. Lookahead `w_right > 0` is still rejected. |
